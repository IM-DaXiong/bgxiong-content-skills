...
Content Skill 是一份**可导入的创作编排包**：不改客户端源码，声明「先规划什么、后规划什么」，在软件里选用后真实产出剧集 / 场次 / 分镜等。
...
1. 设置 → **创作 Skills** → **导入包**（文件夹或 zip）。安装版可先打开随包案例，把 `user-examples/content-skills/` 下某案例拷到可写目录再导入。
...
案例：`10-generation-policy-*`～`13-…`；`14-…` 是故意无效的负例；`15-generation-policy-expression-9grid` 是表情九宫格（9 视角可单图合成一张）。
